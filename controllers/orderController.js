...
export async function getAllOrders(req, res) {
    try{
        //vem os dados da ordem
        const ordersData = await db.query('select * from orders as ors JOIN cakes as cak ON cak.id = ors."cakeid" JOIN clients as cls ON cls.id = ors."clientid"');
        if(ordersData.rowCount == 0){
            return res.sendStatus(404);
        }

        res.send({
            Orders: ordersData.rows.id
        })
